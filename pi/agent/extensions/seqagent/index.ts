...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "- Delegate self-contained work to `worker`: a fix you've already diagnosed, a mechanical refactor, removing a feature, adding tests. Do narrow edits yourself",
      "- `worker` starts cold: it sees none of this conversation, no earlier subagent reports, no plan you presented. Whatever it needs must be in the task text — the files to touch, the change to make, the conventions to follow, and how to verify. It can read files, so point it at them rather than pasting: 'follow the plan in .pi/plans/<slug>.md, steps 3-5' beats a paraphrase, and stays in sync if the plan changes",
      ...
      "- Example: 'implement the plan' → worker has no idea what plan. 'Implement .pi/plans/add-utf-8-slicing.md — all of it; verify with `npm test`' → worker reads it and works from the same source you do",
