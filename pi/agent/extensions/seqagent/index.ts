...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "- `worker` is the only agent that can change anything. The rest are read-only — asking them to edit, delete, or run a build gets you a plan, not the work",
      "- Delegate self-contained work to `worker`: a fix you've already diagnosed, a mechanical refactor, removing a feature, adding tests. Do narrow edits yourself — a one-line change is faster here than a spawned process",
      "- `worker` has no context from this conversation. Spell out which files, what to do, and how to verify. Batch several worker tasks only if they're independent — a later task can't build on an earlier one's output, so staged work (investigate, then fix) needs a second call once you've read the first report",
